...
- **Smaller post-break samples reduce statistical power**, as noted above; the drop in individually-significant tickers should not be read as proof of decay on its own, only the magnitude test (not significant) and the rolling chart (no visible trend break) support that conclusion.
